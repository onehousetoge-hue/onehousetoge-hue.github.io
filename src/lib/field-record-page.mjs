...
export function fieldRecordPage() {
  const body = pageHero({
    ...
    '<section class="field-chapter" id="consultation-booth"><div class="field-chapter-heading"><div><p class="eyebrow">02 · 상담 부스</p><h2>마주하며 듣는<br>어르신의 이야기</h2></div><p>상담 부스에서 어르신과 이야기를 나누는 모습입니다. 현재 이용할 수 있는 상담 내용과 방법은 무료상담 안내에서 확인하실 수 있습니다.</p></div><aside class="photo-context"><h3>현재 상담은 생활의 준비사항을 안내합니다</h3><p>사진 속 배너 문구는 촬영 당시 현장 안내입니다. 한지붕의 현재 무료상담은 임대수익을 보장하거나 입주를 알선하는 서비스가 아닙니다.</p><p>대표와 운영진이 전화·이메일로 빈방 활용과 공동생활을 준비할 때 필요한 기본 정보를 무료로 안내합니다. <a href="/programs/senior-home-consulting/">현재 상담 범위 확인하기</a></p></aside><div class="field-photo-single">' + photoFigure(consultationPhotos[2]) + '</div></section>' +
