...
export function communityActivityPage(activity) {
  ...
  <section class="section"><div class="container content-layout"><article class="prose">
    ...
    ${activity.image ? `<figure class="community-activity-photo"><img src="/assets/activities/${activity.image}" width="${activity.width}" height="${activity.height}" alt="${e(activity.caption)}" loading="lazy" decoding="async"><figcaption>${e(activity.caption)}${activity.photoContext ? `<p class="photo-context-caption">${e(activity.photoContext)}</p>` : ""}</figcaption></figure>` : ""}
