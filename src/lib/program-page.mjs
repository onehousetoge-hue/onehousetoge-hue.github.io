import { consultationImpact, exchangeImpact } from "./impact.mjs";
...
export function programPage(program) {
  ...
  const meta = `<div class="page-meta"><span>${program.status}</span><span>마지막 수정일 ${pageDates[program.href].replaceAll("-", ".")}</span></div><div class="hero-actions"><a class="button" href="${consultation ? "/consultation/" : "/partnership/"}">${consultation ? "홈페이지에서 상담 문의" : "기관협력 문의 보내기"}</a>${consultation ? `<a class="button button-secondary" href="${site.phoneHref}">${icon("phone")}전화로 무료상담 문의</a><a class="button button-secondary" href="${site.emailHref}">${icon("mail")}이메일 작성하기</a>` : `<a class="button button-secondary" href="/contact/">${research ? "조사 협력 문의" : "전화·이메일 문의"}${icon("arrow")}</a><a class="button button-secondary" href="/resources/">생활자료 보기</a>`}</div><p class="field-hint">${consultation ? "현재 전화·이메일로 안내하며, 입주 연결·계약대행은 제공하지 않습니다." : "기관 요청에 따라 사전 협의하며, 문의만으로 일정이나 참여가 확정되지는 않습니다."}</p><a class="text-link" href="/contact/#inquiry-templates">무엇을 적을지 막막하다면, 문의 문안 보기${icon("arrow")}</a>`;
  const shortcuts = `<nav class="section-jumps" aria-label="이 페이지에서 바로 찾기"><p>필요한 내용부터 확인하세요</p><div><a href="#program-contents">${consultation ? "상담 내용" : "프로그램 내용"}</a><a href="#program-how">이용방법</a><a href="#program-cost">비용</a><a href="#program-limits">제공하지 않는 업무</a><a href="#program-faq">자주 묻는 질문</a></div></nav>`;
  ...
  const extra = consultation
    ? `<h2>상담 후 얻을 수 있는 도움</h2><p>바로 계약하거나 결정을 내리기보다, 가족과 더 이야기할 항목, 확인이 필요한 정보, 참고할 생활자료를 정리하는 데 도움을 받을 수 있습니다.</p><h2>상담 전에 준비할 내용</h2><p>궁금한 점과 현재 고민을 간단히 알려주세요. 처음 문의하실 때 주민등록번호, 상세 주소, 등기·계약 서류, 금융정보를 보내실 필요는 없습니다. 추가 정보가 필요하다면 담당자가 목적과 범위를 설명한 뒤 요청합니다.</p><p><a href="/resources/consultation-preparation/#prepare">상담에서 물어볼 질문 정리하기</a></p>`
    : research
  ...
  const body = `${pageHero({ eyebrow: `공익사업 ${program.order}`, title, description: detail.subtitle, meta })}
    ...
    </article><aside class="side-nav"><h2>${consultation ? "무료상담 문의" : research ? "조사 협력 문의" : "기관협력 문의"}</h2><a href="${consultation ? "/consultation/" : "/partnership/"}">홈페이지에서 문의 보내기</a><a href="${site.phoneHref}">${site.phone}</a><a href="${site.emailHref}">${site.email}</a><a href="/contact/">문의 방법 보기</a><a href="/participate/#partnership">참여·기관협력 안내</a><a href="/programs/">공익사업 전체 보기</a></aside></div>
    <div class="container contact-strip"><div><h2>${consultation ? "궁금한 점부터 이야기해 주세요." : research ? "조사 협력을 문의해 주세요." : "교육·봉사를 함께 기획하고 싶으신가요?"}</h2><p>민감정보나 상세주소 없이 궁금한 내용부터 알려 주세요.</p></div><div class="contact-actions"><a class="button" href="${consultation ? "/consultation/" : "/partnership/"}">홈페이지에서 문의 보내기</a><a class="button button-secondary" href="${site.phoneHref}">전화 문의</a><a class="button button-secondary" href="${site.emailHref}">이메일 작성하기</a></div></div></section>`;
  return layout({ title: program.title, description: detail.subtitle, path: program.href, body, updatedAt: pageDates[program.href], breadcrumbs: [{ label: "공익사업", href: "/programs/" }, { label: program.title, href: program.href }] });
}
