...
export function researchPage(program) {
  const body = pageHero({
    ...
    meta: `<div class="page-meta"><span>현황 기준일 <time datetime="${research.asOf}">2026년 9월 24일</time></span><span>상세 현황 게시일 <time datetime="${research.publishedAt}">2026년 9월 24일</time></span></div><div class="hero-actions"><a class="button" href="/contact/">실태조사 의견 전달 문의</a><a class="button button-secondary" href="#questions">조사 질문 확인하기</a></div>`,
    ...
    <h2 id="overview">조사 개요</h2><dl class="definition-list"><div><dt>조사기간</dt><dd><time datetime="${research.start}">2026년 9월</time>~<time datetime="${research.end}">11월 30일</time></dd></div><div><dt>조사지역</dt><dd>${escapeHtml(research.area)}</dd></div><div><dt>조사대상</dt><dd>지역 어르신, 어르신 가족, 청년, 복지기관 관계자</dd></div><div><dt>조사방법</dt><dd>${escapeHtml(research.methods)}</dd></div><div><dt>현재 단계</dt><dd>${escapeHtml(research.stage)}</dd></div><div><dt>현황 기준일</dt><dd>2026년 9월 24일</dd></div></dl>
    <section id="progress" aria-labelledby="progress-title" class="research-progress"><p class="eyebrow">2026년 9월 24일 기준 · 운영진 집계</p><h2 id="progress-title">현재까지 ${researchTotal}명의 의견을 들었습니다</h2><dl class="research-counts">${research.participants.map((group) => `<div><dt>${escapeHtml(group.label)}</dt><dd>${group.count}명</dd></div>`).join("")}</dl><p><strong>전체 의견수집 ${researchTotal}명</strong> · 동일인이 여러 차례 의견을 제공한 경우 한 명으로 집계했습니다. 상담 기록의 회차별 참여인원과는 집계 범위가 다릅니다.</p><p>이 수치는 의견수집 참여 현황이며, 지역 전체를 대표하는 통계나 공동거주 성과가 아닙니다.</p></section>
    <p>주요 조사 문항은 18개이며, 현장 조사 및 인터뷰는 9회 진행했습니다. 아래 목록은 조사 주제별 안내이며 설문지 18개 문항의 전문은 아닙니다.</p><h2 id="questions">무엇을 조사하고 있나요?</h2>${research.topics.map((topic, index) => `<section class="research-topic"><h3>${index + 1}. ${escapeHtml(topic.title)}</h3>${list(topic.questions)}</section>`).join("")}
  ...
  return layout({ title: research.title, description: "노원구 공릉동·하계동에서 진행 중인 고령가구 유휴공간 및 세대공존 실태조사. 2026년 9월 24일 기준 127명의 의견수집 현황과 조사 질문·일정을 안내합니다.", path: program.href, body, updatedAt: research.publishedAt, breadcrumbs: [{ label: "공익사업", href: "/programs/" }, { label: "주거상생 실태조사", href: program.href }] });
}
