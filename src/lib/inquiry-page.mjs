...
export function inquiryPage(kind) {
  ...
  return layout({title,description:partner ? '복지관·경로당·학교·지역기관의 디지털 교육, 봉사, 주거문화 안내와 조사 협력 문의입니다.' : '어르신과 가족의 빈방 활용·공동생활 준비에 관한 무료상담 문의입니다. 이름과 회신 연락처 하나, 궁금한 내용만 남겨 주세요.',path:route,body:`<div class="inquiry-page">${body}</div>`,breadcrumbs:[{label:title,href:route}]});
}
