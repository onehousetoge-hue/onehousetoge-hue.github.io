...
export function fundingPage(activity) {
  const body = pageHero({
    ...
    <h2>지원금과 현재 무료상담은 이렇게 구분합니다</h2><p>위 금액은 어르신에게 청구하는 상담료가 아닙니다. 현재 전화·이메일 기초상담은 한지붕 대표와 운영진이 무료로 응대합니다. 기관·경로당 상담은 사전 협의하며, 주택 방문상담의 과거 실적과 현재 접수 범위는 구분합니다. 안전진단은 제공하지 않습니다.</p>
