...
const programsBody = `${pageHero({ eyebrow: "공익사업", title: "지금 받을 수 있는 도움과<br>진행 중인 주거상생 실태조사", description: "대표와 운영진의 무료상담, 기관 요청에 따른 교육·봉사를 제공합니다. 청년 주거와 고령층 유휴공간에 관한 실태조사를 진행하고 있습니다.", meta: updatedMeta })}<section class="section"><div class="container"><div class="program-grid">${programCards(2)}</div><div class="notice"><h2>한지붕이 하지 않는 일</h2><p>한지붕은 부동산 중개, 입주·계약 보장, 공사와 안전진단, 개별 법률·세무 자문을 제공하지 않습니다. 필요한 경우 관계기관 또는 자격 있는 전문가에게 확인할 수 있도록 구분해 안내합니다.</p></div></div></section>`;
await emit("/programs/", layout({ title: "공익사업", description: "어르신 유휴공간·빈방 활용 무료상담, 기관 협의형 교육·봉사와 진행 중인 주거상생 실태조사를 안내합니다.", path: "/programs/", body: programsBody, breadcrumbs: [{ label: "공익사업", href: "/programs/" }] }));

for (const program of programs) await emit(program.href, programPage(program));

const resourcesBody = `${pageHero({ eyebrow: "주거상생 자료", title: "가족과 이야기할 때,<br>함께 생활할 때 꺼내 보세요.", description: "가족회의 질문지부터 생활규칙 작성표까지, 각자의 고민에 맞춰 읽고 직접 적어볼 수 있는 생활자료 6종입니다. 메모는 자동 저장되거나 전송되지 않습니다.", meta: updatedMeta })}<section class="section"><div class="container"><div class="resource-grid">${resourceCards(resources, 2)}</div></div></section>`;
